...
public class Timer {
    ...
    public <T, U> double repeat(int n, Supplier<T> supplier, Function<T, U> function, UnaryOperator<T> preFunction, Consumer<U> postFunction) {
        ...
        for (int i=1; i<=n; i++ ){
            pauseAndLap();
            T supplierTime = supplier.get();
            U functionTime =null;
            if(preFunction!= null) {
                T preprocessedTime = preFunction.apply(supplierTime);
                resume();
                functionTime = function.apply(preprocessedTime);
            }else{
                resume();
                functionTime = function.apply(supplierTime);
            }
            pause();
            if(postFunction!= null) {
                postFunction.accept(functionTime);
            }
            resume();
        }
        //pause without counting a lap
        pause();
        Double meanTime = 0.00;
        //finally return result of calling meanlaptime
